...
class Glass:
    def __init__(self, shrinkage_factor=0, ngroup = 2, dtype = tf.float32) -> None:
    ...
    def process_data(self, X_train: np.ndarray, y_train: np.ndarray):
        ...
        def jointmodel():
            sigma = yield tfd.Sample(tfd.HalfCauchy(0.0, tf.cast(1, self.dtype)), self.ngroup-1)
            # weights
            beta = yield tfb.Cumsum()(tfd.Sample(tfd.Normal(0.0, 1.0), (self.ngroup-1, self.nT)))
            beta = sigma[:, None] * beta
            beta = tfp.math.soft_threshold(beta, self.shrinkage_factor)
            probs = yield tfd.RelaxedOneHotCategorical(1, logits=tf.zeros([self.nchannel, self.ngroup]))
            weights = yield tfd.Sample(tfd.Normal(0.0, 1.0), [self.nchannel, self.ngroup-1])
    ...
    def mfvb(self, num_steps=5000, sample_size=10, importance_sample_size=10, learning_rate=0.05, seed=1):
        self.posterior = tfd.JointDistributionSequentialAutoBatched([
            tfd.LogNormal(
                tf.Variable(tf.zeros(self.ngroup-1, dtype = self.dtype) - 3),
                tfp.util.TransformedVariable(0.1 * tf.ones(self.ngroup-1, dtype = self.dtype), bijector = tfb.Softplus())),
            tfd.Normal(
                tf.Variable(tf.random.normal((self.ngroup-1, self.nT), stddev=3*self.shrinkage_factor, dtype = self.dtype), 
                            dtype = self.dtype),
                tfp.util.TransformedVariable(0.01 * tf.ones((self.ngroup-1, self.nT), dtype = self.dtype), 
                                             bijector = tfb.Softplus())),
            tfd.RelaxedOneHotCategorical(0.5, logits=tf.Variable(tf.zeros((self.nchannel, self.ngroup)))),
            tfd.Normal(tf.Variable(tf.zeros([self.nchannel, self.ngroup-1], dtype = self.dtype), dtype = self.dtype),
                       tfp.util.TransformedVariable(tf.ones([self.nchannel, self.ngroup-1], dtype = self.dtype), 
                                                    dtype = self.dtype,
                                                    bijector = tfb.Softplus()))
        ])
